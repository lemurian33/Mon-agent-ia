...
type LogicStep =
  ...
  | { type: "action"; app: "report" | "pipeline" | "hr"; title: string; sub: string };
...
const MESSAGES: Message[] = [
  {
    id: 1,
    from: "user",
    text: "Génère le rapport financier du trimestre et identifie les postes de dépenses à optimiser.",
    time: "9:03",
  },
  {
    id: 2,
    from: "agent",
    text: "Rapport Q3 généré. 3 postes d'optimisation identifiés — économie potentielle estimée à 18 400 €. Synthèse envoyée à la direction.",
    time: "9:04",
  },
  {
    id: 3,
    from: "user",
    text: "Lance aussi une campagne de relance sur nos 40 prospects B2B en attente depuis 30 jours.",
    time: "9:06",
  },
  {
    id: 4,
    from: "agent",
    text: "Campagne lancée sur 40 prospects. Séquence email personnalisée x3, rappels planifiés. 6 créneaux de démo déjà réservés automatiquement.",
    time: "9:07",
  },
];
...
const STEPS: LogicStep[] = [
  {
    ...
    cards: [
      { icon: <ChartIcon />, label: "Tâche détectée", value: "Rapport financier Q3" },
      { icon: <ScanIcon />, label: "Analyse en cours", value: "Dépenses — 12 postes scannés" },
    ],
  },
  {
    type: "action",
    app: "report",
    title: "Rapport généré & envoyé",
    sub: "Économie potentielle : 18 400 €",
  },
  {
    type: "thinking",
    label: "réflexion agent",
    cards: [
      { icon: <UsersIcon />, label: "Pipeline ciblé", value: "40 prospects B2B — J+30" },
      { icon: <MailIcon />, label: "Action", value: "Séquence email x3 + rappels" },
    ],
  },
  {
    type: "action",
    app: "pipeline",
    title: "6 démos réservées automatiquement",
    sub: "Pipeline commercial relancé",
  },
];
    ...
    <div ref={sectionRef} className="flex flex-col gap-3 mx-auto max-w-6xl">
      ...
      <div className="mx-auto max-w-2xl text-center mb-8">
        ...
        <h2 className="mt-4 text-3xl font-semibold tracking-tight text-balance text-foreground sm:text-4xl">
          Vos équipes se concentrent.{" "}
          <span className="text-orange-500">Vos agents exécutent.</span>
        </h2>
        <p className="mt-4 text-lg text-muted-foreground text-pretty">
          Finance, RH, commercial — chaque département dispose de ses propres agents spécialisés,
          coordonnés en temps réel. Résultats mesurables dès la première semaine.
        </p>
      </div>
        ...
        <div className="flex w-[320px] shrink-0 items-center justify-center p-5">
          <div className="w-[248px] overflow-hidden rounded-[32px] border-[2.5px] border-zinc-800 bg-zinc-900 shadow-2xl">
            ...
            <div className="flex items-center gap-2 border-b border-zinc-800 bg-zinc-900 px-2 py-1.5">
              ...
              <div>
                <div className="text-[11px] font-semibold text-white">Enterprise Hub</div>
                <div className="text-[8px] text-green-400">en ligne</div>
              </div>
            </div>

            {/* Messages */}
            <div className="flex min-h-[290px] flex-col gap-2 px-2 py-2.5" style={{ background: "#0f1117" }}>
              ...
              {MESSAGES.map((msg, i) => (
                ...
                >
                  <div
                    className="max-w-[140px] px-2.5 py-1.5 text-[9px] leading-[1.4] text-white"
                    style={{
                ...
                {step.type === "thinking" ? (
                  ...
                  <div className="flex items-center gap-3 rounded-lg border border-border bg-background px-3 py-2.5">
                    <div
                      ...
                      style={{
                        background:
                          step.app === "report"   ? "#EAF3DE" :
                          step.app === "pipeline" ? "#FAEEDA" :
                                                    "#EAE8F3",
                      }}
                    >
                      {step.app === "report"   ? <ReportIcon /> :
                       step.app === "pipeline" ? <PipelineIcon /> :
                                                 <HrIcon />}
                    </div>
                    <div>
...
function ScanIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 20 20" fill="none">
      <path d="M2 7V4a1 1 0 011-1h3M14 3h3a1 1 0 011 1v3M18 13v3a1 1 0 01-1 1h-3M6 17H3a1 1 0 01-1-1v-3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/>
...
function ReportIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      <path d="M2 14l4-4 4 2 4-6 4 2" stroke="#27500A" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M2 18h16" stroke="#27500A" strokeWidth="1.3" strokeLinecap="round"/>
    </svg>
  );
}
...
function PipelineIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      <circle cx="4" cy="10" r="2" stroke="#633806" strokeWidth="1.3"/>
      <circle cx="10" cy="10" r="2" stroke="#633806" strokeWidth="1.3"/>
      <circle cx="16" cy="10" r="2" stroke="#633806" strokeWidth="1.3"/>
      <path d="M6 10h2M12 10h2" stroke="#633806" strokeWidth="1.3" strokeLinecap="round"/>
    </svg>
  );
}
...
function HrIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      <circle cx="10" cy="7" r="3" stroke="#3b2d8a" strokeWidth="1.3"/>
      <path d="M3 18c0-4 3-6 7-6s7 2 7 6" stroke="#3b2d8a" strokeWidth="1.3" strokeLinecap="round"/>
    </svg>
  );
}
